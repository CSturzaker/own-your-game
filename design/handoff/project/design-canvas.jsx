...
function DesignCanvas({ children, minScale, maxScale, style }) {
  ...
  React.useEffect(() => {
    ...
    // Programmatic focus: window.dispatchEvent(new CustomEvent('dc:focus',{detail:{id:'art-id'}}))
    // Accepts either an artboard id (we'll resolve which section it belongs to)
    // or a full "section/artboard" slot id.
    const onFocus = (e) => {
      const want = e.detail && e.detail.id;
      if (!want) { api.setFocus(null); return; }
      if (registry[want]) { api.setFocus(want); return; }
      // Look up the artboard id across all section meta
      for (const sid of sectionOrder) {
        const meta = sectionMeta[sid];
        if (meta && meta.slotIds && meta.slotIds.includes(want)) {
          api.setFocus(`${sid}/${want}`);
          return;
        }
      }
    };
  ...
  }, [api, registry, sectionMeta, sectionOrder]);
