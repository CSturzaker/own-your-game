...
| `PUBLIC_STREAM_MONTAGE_UID`        | public | `src/lib/stream.ts` → campaign film (DEV-124)    | The Stream UID of the home-page campaign montage (same account, same subdomain). Soft accessor — unset until the asset is produced: the poster renders, play shows the unavailable state. Never set a placeholder.                                                |
| `PUBLIC_CF_BEACON_TOKEN`           | public | `src/lib/analytics.ts` → `BaseLayout` (DEV-125)  | Cloudflare Web Analytics site token (dashboard → Web Analytics → manage snippet). Public — it ships in `data-cf-beacon`. Set ONLY in the Pages **Production** env; unset everywhere else (Preview, CI, local) so the beacon is absent from non-production builds. |
